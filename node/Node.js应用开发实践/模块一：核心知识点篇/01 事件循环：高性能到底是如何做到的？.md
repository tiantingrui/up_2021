...
**注意：**是基于 node 10+ 版本，讲解事件循环的原理。

这个**事件循环原理**和 **浏览器的原理**是不同的。Node.js 10+ 版本后虽然在运行结果上与浏览器一致，但是两者在原理上一个是基于浏览器，一个是基于 **libev 库**。浏览器核心的是宏任务和微任务，而在 Node.js 还有**阶段性任务执行阶段。**
...
在事件循环原理官网的图片中可以看出来，起点事timers，如下代码所示：
...
浏览器的事件循环原理和 Node.js 事件循环原理的区别以及联系有哪些点？